...
export function ProgressChart({ course }: { course: CourseDetailsDto }) {
  ...
  const hasNoActorData =
    data && Object.values(data?.actor).every((v) => v === 0);
  ...
  if (hasNoActorData) {
    return (
      <p className="p-4 text-sm text-muted-foreground">
        You haven't started this course yet, so there are no metrics available.
      </p>
    );
  }
  return (
